...
class TokenRefreshMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        """Метод обрабатывает входящий запрос и автоматически обновляет Access
        Token, если он устарел или вызывает ошибку, если токен недействителен.

        Последовательность действий:
        1. Проверяется наличие и валидность текущего Access Token.
        2. Если токен устарел или отсутствует, генерируется
                                    новый Access Token с помощью Refresh Token.
        3. После успешного обновления новый токен сохраняется
                        в состоянии запроса (`request.state.new_access_token`).
        4. Далее запрос продолжает путь обработки через
                                    следующий слой middleware (call_next).
        5. Перед отправкой клиенту ответ дополняется новым
                                    Access Token в куки, если он был обновлён.

        Важно:
        Токен добавляется в куки после завершения обработки основного
        маршрута, чтобы обеспечить согласованность состояний.

        Если новый токен был сгенерирован, он временно сохраняется
        в `request.state.new_access_token` до завершения цепочки middleware,
        чтобы быть установленным в куки сразу — без необходимости повторного
        запроса от клиента. Это позволяет клиенту получить обновлённый
        токен в этом же ответе, и сразу видеть, что он аутентифицирован.
    ...
    def _maybe_set_new_access_token(
        self,
        request: Request,
        response: Response,
    ) -> None:
        ...
        if hasattr(request.state, "new_access_token"):
            self._set_new_access_token(
                response=response, token=request.state.new_access_token
            )
